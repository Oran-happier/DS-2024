...
#define DEFAULT_CAPACITY  5000 //默认的初始容量（实际应用中可设置为更大）
...
public:
	...
	{
		_elem = new T[_capacity = c]; _size = c;//_size = 0
	}
...
{
	T e[DEFAULT_CAPACITY];//创建一个副本
	for (unsigned i = 0; i < _size; i++) e[i] = _elem[i];

	clock_t start = 1000*clock();
	bubbleSort(lo, hi);
	clock_t end = 1000*clock();
	...
	_elem = e;//还原
	std::cout << "bubbleTime:" << bubbleTime << std::endl;
	std::cout << "mergeTime:" << mergeTime << std::endl;
}
