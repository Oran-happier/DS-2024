...
{//列表模板类
private:
	Rank_size; ListNodePosi<T> head, tail;//规模、头哨兵、尾哨兵
...
public:
	...
	bool empty() const { return_size <= 0; }//判空
	...
	void merge(List<T>& L) { merge(head->succ, _size, L, L.head->succ, L._ size); }//全列表归并
...
ListNodePosi<T> List<T>::find(T const& e, Rank n, ListNodePosi<T> p) const {
	while (0 < n--)//(0 <= n <= Rank(p) <_ size)对于p的最近的n个前驱,从右向左
...
template <typename T> T List<T>::remove(ListNodePosi<T> p) { //删除合法节点p
	Te = p->data;//备份待删除节点的数值(假定T类型可直接赋值)
...
template <typename T> Rank List<T>::dedup() {
	Rank oldSize = size; ListNodePosi<T> p = first();
...
ListNodePosi<T> List<T> ::search(T const& e, Rank n, ListNodePosi<T> p) const {
	do {//初始有:0 <= n <= rank(p) <_ size;此后,n总是等于p在查找区间内的秩
		p = p->pred; n--; //从右向左
...
ListNodePosi<T> List<T>::merge(ListNodePosi<T> p, Rank n,
	...
	// L.valid(q) && Rank(q) + m <= L ._ size && L.sorted(q, m)
