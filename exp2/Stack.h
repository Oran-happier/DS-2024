...
{
public://开放式接口，直接使用
	void push(T const& e) { Vector<T>::insert(Vector<T>::size(), e); }//入栈：将新元素作为向量的末元素插入
...
{	//对(已剔除白空格的)表达式s求值,并转换为逆波兰式RPN
	...
	while (!optr.empty() && !br) {//在运算符栈非空之前,逐个处理表达式中各字符
		if (isdigit(*S)) {//若当前字符为操作数,则
			...
			case'>': {//栈顶运算符优先级更高时,可实施相应的计算,并将结果重新入栈
				...
				{//对于其它(二元)运算符
					...
					{
						//由于Vector预设的是5大小，所以所有的操作都会在6开始
						// 当5都被取出时，说明预先压入的'\0'已经弹出,并且opnd被取走了空和最终结果
						//*1、选取此时作为弹出的时机，但是pop()的定义是会修改opnd本体的
						// 为了保留不该被删除的数据，所以采用[]读取的方式
						//*2、如果确实需要计算，那么就会在下面完成，那时再用pop修改opnd
						br = true;
					}
					else
					{
						opnd2 = opnd.pop(), opnd1 = opnd.pop();//取出后、前操作数
