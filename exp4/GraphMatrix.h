...
private:
	...
	//Rank& n = Graph<Tv, Te>::n;
	//Rank& e = Graph<Tv, Te>::e;
public:
...
// 顶点的动态操作
	...
	{ //删除第v个顶点及其关联边（0 <= v < n）
		for (Rank u = 0; u < Graph<Tv, Te>::n; u++) //所有
			if (exists(v, u)) //出边
			{
				delete E[v][u]; V[u].inDegree--; Graph<Tv, Te>::e--;
			} //逐条删除
		...
		for (Rank u = 0; u < Graph<Tv, Te>::n; u++) //所有
			...
			{
				delete x; V[u].outDegree--; Graph<Tv, Te>::e--;
			} //逐条删除
...
// 边的基本操作：查询顶点v与u之间的联边（0 <= v, u < n且exists(v, u)）
	virtual EType& type(Rank v, Rank u)
	{ 
		return E[v][u]->type; 
	} //边(v, u)的类型
